...
class CssProperties:
    ...
    def add_text(self, text: str, id: str = None):
        ...
        :param id: Unique reference for this CSS fragment
        """
        if id is None:
            self._context['css']["text"].append(text)
        else:
            if id not in self.__uniq_text:
                self._context['css']["text"].append(text)
                self.__uniq_text.add(text)
...
class Report:
    ...
    def __init__(self, inputs: dict = None, script: str = None):
        ...
        The target of this components is not to replaced libraries but only to easy the use of the most popular ones
