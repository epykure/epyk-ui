...
class CssInputInteger(CssStyle.Style):
  ...
  """
    cssId = {
        'reference': 'input[type=number]::-webkit-inner-spin-button, input[type=number]::-webkit-outer-spin-button'}
    _attrs = {"-webkit-appearance": 'none', "margin": 0}
...
class CssAutocomplete(CssStyle.Style):
    ...
    def customize(self):
        self.css({
            "z-index": 220, 'background-color': self.page.theme.colors[0], 'color': self.page.theme.greys[5]},
            important=True)
