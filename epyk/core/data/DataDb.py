...
from ..py import primitives, PySql
from ..js.imports.utils import requires


MISSING_PACKAGE: str = 'Missing Package'
...
class DataDb:
    ...
    def private(self) -> PySql.SqlConn:
        ...
        if database not in self._db_bindings:
            ...
            if settings["family"] == "sqlite":
                ...
                db = self.oracle(
                    settings["private"], settings["host"], settings["port"], model_path=model_path,
                    tables_scope=self.table_names)
    ...
    def public(self) -> PySql.SqlConn:
        ...
        if database not in self._db_bindings:
            ...
            if settings["family"] == "sqlite":
                ...
                db = self.mssql(settings["public"], settings["host"], driver_name=settings["driverName"],
    ...
    def oracle(self, name: str, host: str, port: int, model_path: str = None, is_secured: bool = False,
        ...
        if database not in self._db_bindings:
            self._db_bindings[database] = PySql.SqlConn('oracle', database=database, tables_scope=tables_scope,
                                                        **db_settings)
        return self._db_bindings[database]
    ...
    def accdb(self, name, db_path, model_path=None):
        """Get an Access (.accdb) Database query object using ODBC driver.
