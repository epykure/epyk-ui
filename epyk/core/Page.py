...
from epyk.core.css import themes, Classes, Icons

from epyk.core import html, js, data, auth
from epyk.core.html import symboles, entities, skins
from epyk.core.py import OrderedSet, PyExt
...
class Report:
    ...
    def __init__(self, inputs: dict = None, script: str = None):
        ...
        self._props = {'js': {
            ...
            # Loaded frameworks
            'fwks': {},
            # Loaded pyks (components extensions)
            'pyks': {},
    ...
    def imports(self) -> ImportManager:
        ...
        if self.__import_manage is None:
            self.__import_manage = ImportManager(page=self)
    ...
    def web(self) -> Interface.WebComponents:
        ...
        `w3schools <https://www.w3schools.com/html/default.asp>`_
        """
        return Interface.WebComponents(self)
    ...
    def licenses(self, commercial: bool = False) -> dict:
        ...
        for pkg in self.jsImports:
            packages[pkg] = all_js[pkg].get('license')
            plans = all_js[pkg].get('pricing')
            if plans and commercial:
                packages.setdefault("commercial", {})[pkg] = plans
        for pkg in self.cssImport:
            if pkg not in all_js:
                if pkg in all_css:
                    packages[pkg] = all_css[pkg].get('license')
                    plans = all_css[pkg].get('pricing')
                    if plans and commercial:
                        packages.setdefault("commercial", {})[pkg] = plans
        return packages
