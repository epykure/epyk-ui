...
class SqlConn:
    ...
    """
        ...
        dbConfig = {'drivername': kwargs["driver"] if "driver" in kwargs else family,
        ...
        if kwargs.get('driver', '') == "mssql+pyodbc":
            ...
            try:
                params = urllib.parse.quote_plus(
                    'DRIVER=' + kwargs.get('driverName', "{ODBC Driver 17 for SQL Server}") +
                    ';SERVER=' + kwargs['host'] + ';DATABASE=' + database +
                    ';Trusted_Connection=yes;')
            except:
                params = urllib.quote_plus(
                    'DRIVER=' + kwargs.get('driverName', "{ODBC Driver 17 for SQL Server}") + ';SERVER='
                                ...
                                if c.default is not None:
                                    self.engine.execute(
                                        'ALTER TABLE %s ADD COLUMN %s %s DEFAULT %s' % (
                                            table_name, cn, c.type.compile(self.engine.dialect),
                                            json.dumps(c.default.arg)))
                                else:
                                    self.engine.execute(
