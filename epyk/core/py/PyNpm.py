...
from epyk.core.js.imports.utils import script_cdnjs_path, script_npm_path
...
class NpmRegistery:
    __HTTP_GITHUB_USER: str = "https://raw.githubusercontent.com"
    __HTTP_GITHUB: str = "https://api.github.com"
    __HTTP_NPM_REGISTERY: str = 'https://registry.npmjs.org'
    ...
    def has_cdnjs(self, version: Optional[str] = None) -> list:
        ...
        results = []
        for script in get_js()[self._alias]['modules']:
        ...
        for script in get_css().get(self._alias, {}).get('modules', []):
            script["path"] = script["path"] % {"version": version}
            path = "%s/%s%s" % (script["cdnjs"], script["path"], script["script"])
            try:
                urlopen(path)
    ...
    def repository(self):
        ...
        if repo_path is None:
            repo_path = get_js()[self._alias].get("repository")
        return repo_path
    ...
    def info(self, version=None):
        ...
        if self._info is None:
            version = version or self.version_no
            headers = {"Content-Type": 'application/json', 'Accept': 'application/json', 'Connection': 'keep-alive'}
            request = Request("%s/%s/%s" % (self.__HTTP_NPM_REGISTERY, self._alias, version), method="GET",
                              headers=headers)
    ...
    def version_no(self):
        ...
        all_js_configs = get_js()
        v = all_js_configs[self._alias]['modules'][0].get(
            "version", all_js_configs[self._alias].get('version', self.release))
        return v

    @property
    def version(self) -> str:
        """ Get the package version tag (it is either the version number of the version number prefixed with v). """
        prefix = get_js()[self._alias].get("v_prefix")
    ...
    def scripts(self) -> list:
        ...
        This framework will only import directly from a pure vanilla JavaScript implementation the various modules
        mandatory to benefit from those external packages.
        ...
        """
        results = []
        for script_type in [get_css(), get_js()]:
    ...
    def get_files(
        ...
        """ Get all the files defined from the package.json files to setup correctly the package from a Node.js server.

        The framework will use this setup in order to ease the transition and compatibility with existing popular
        framework.

        :param version: Optional. The destination path for the scripts (example the Node.js modules path)
        :param out_path: Optional. Output path for files
            ...
            for reg_exp in regs:
                ...
                if match_reg is not None:
                    if out_path is not None:
                        ...
                        out_file = os.path.join(str(folder_path), t['path'].split("/")[-1])
                        if os.path.exists(out_file):
                            if update:
                                ...
                                if verbose:
                                    logging.warning(
                                        "{} - {} File already available".format(
                                            self._alias, t['path'].split("/")[-1]))
                        else:
                            json_config = req.read().decode(req.info().get_content_charset())
                            with open(out_file, "w", encoding="utf-8") as f:
                                f.write(json_config)
...
class Npm:
    __HTTP_NPM_REGISTRY: str = 'https://registry.npmjs.org'
    ...
    def all(self, verbose: bool = True):
        ...
        :param verbose: Optional. Display warning message. Default True.
        """
        return {js: self.check(js, verbose) for js, js_details in get_js().items()}
...
class Packages:
    ...
    @classmethod
    def descriptions(cls, verbose: bool = True) -> dict:
        ...
        npm = Npm()
        results = {}
        for js, js_details in get_js().items():
            pkg = npm.package(js)
            if pkg is not None:
    ...
    @classmethod
    def versions(cls, verbose: bool = True) -> dict:
    ...
    @classmethod
    def repositories(cls, verbose: bool = True) -> dict:
...
def download(modules_path: str, update: bool = False, verbose: bool = True, packages: Optional[list] = None,
...
def install(path: str, packages: list = None, node_server: bool = False, update: bool = False, verbose: bool = True,
  ...
  Usage::
    PyNpm.install("C:\\tmps\\packages", ['bootstrap'], update=True)
  ...
  :param node_server: Boolean. Optional. Specify if npm from Node.js must be used to install the package
  ...
  """
    ...
    if node_server:
        ...
        all_js_configs = get_js()
        all_css_configs = get_css()
        for p in packages:
            if not os.path.exists(os.path.join(path, p)) or update:
                for category in [all_js_configs, all_css_configs]:
                    for mod in category.get(p, {}).get('modules', []):
                        request = Request(script_cdnjs_path(p, mod))
                        response = urlopen(request).read()
                        script_path = script_npm_path(p, mod, path)
