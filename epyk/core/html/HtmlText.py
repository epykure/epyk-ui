...
class Text(Html.Html):
  ...
  def __init__(self, report, text, size, color, align, width, height, htmlCode, tooltip, helper, profile):
  ...
  def onDocumentLoadFnc(self):
    self.addGlobalFnc("%s(htmlObj, data, jsStyles)" % self.__class__.__name__, '''
      if (jsStyles.reset){htmlObj.html(%(markUp)s(data))} 
      else {
        if(data != ''){ 
          if ((jsStyles.maxlength != undefined) && (data.length > jsStyles.maxlength)){
            var div = $("<div title='"+ data +"' data-html='true'>" + %(markUp)s(data.slice(0, jsStyles.maxlength)) + "...<i class='fas fa-times' onclick='$(this).parent().remove()' style='margin-left:10px;cursor:pointer'></i></div>") ;
            htmlObj.append(div)} 
          else{
            var div = $("<div>" + %(markUp)s(data) + "<i class='fas fa-times' onclick='$(this).parent().remove()' style='margin-left:10px;cursor:pointer'></i></div>") ; 
            htmlObj.append(div)}}}''' % {"markUp": self._report.js.fncs.markUp})
...
class Code(Html.Html):
  ...
  def __init__(self, report, vals, size, color, width, height, edit, htmlCode, helper, profile):
    ...
    self.css({'color': self.color, 'display': 'block', 'font-size': "%s%s" % (size[0], size[1]), 'margin': '5px 0 5px 0'})
...
class Pre(Html.Html):
  ...
  def __init__(self, report, vals, size, color, width, height, htmlCode, dataSrc, helper, profile):
  ...
  def onDocumentLoadFnc(self):
    self.addGlobalFnc("%s(htmlObj, data, jsStyles)" % self.__class__.__name__, "htmlObj.empty(); htmlObj.html(data)")
...
class Title(Html.Html):
  ...
  def __init__(self, report, text, size, level, name, contents, color, picture, icon, marginTop, htmlCode, width,
               height, align, profile):
    jsStyles, tmpText = None, {"reset": True, 'markdown': False}
