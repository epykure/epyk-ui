...
class Navigation:
    ...
    """
        ...
        du = self.page.ui.icon(icon, width=width, height=height, options=options, profile=profile).css(
            {"border": '1px solid black', "position": 'fixed', "width": 'none', "border-radius": '20px',
             "padding": '8px',
             "right": '%spx' % right})
        if top is not None:
    ...
    """
        ...
        self.page.js.onReady(
            self.page.js.window.events.addScrollListener([
                self.page.js.if_(
                    self.page.js.window.scrollY < (self.page.js.window.scrollMaxY - 50), [dd.dom.show()]).else_(
                    dd.dom.hide())
            ]))
        if tooltip is not None:
            dd.tooltip(tooltip)
    ...
    """
        height = Arguments.size(height, unit="px")
        p = self.page.ui.sliders.progressbar(progress, width=width, height=height, options=options, profile=profile,
                                             html_code=html_code)
        self.page.js.onReady([
            self.page.js.window.events.addScrollListener([
        ...
        for rec in record[:-1]:
            ...
            div += self.page.ui.text(divider).css(
                {"display": 'inline-block', 'margin': '0 5px',
                 'font-size': self.page.body.style.globals.font.normal(-2)})
    ...
    def nav(self, logo=None, title: str = None, components=None, size: Union[tuple, int] = (40, 'px'),
            options: dict = None, avatar: bool = False, html_code: str = "page_nav_bar",
            profile: Union[dict, bool] = False
    ...
    def footer(self, components=None, logo=None, width: Union[tuple, int] = (100, '%'),
               height: Union[tuple, int] = (40, 'px'),
    ...
    def side(self, components=None, anchor=None, size=262, position='right', options=None, profile=False,
             z_index: int = 20, overlay: bool = False, padding: int = None,
             push: bool = False) -> html.HtmlContainer.Div:
        """
...
class Banners:
    ...
    def bottom(self, data: Union[str, list] = "", background=None, align="center", width=(100, '%'),
               height=(None, 'px'),
    ...
    def info(self, data, icon="fas fa-info-circle", background=None, width=(100, '%'), height=(None, 'px'),
             options=None,
             profile=None):
        """
    ...
    def title(self, title, content, size_notch=0, background=None, width=(100, '%'), align="center",
              height=(None, 'px'),
              options=None, profile=None):
        """
    ...
    """
        ...
        if not hasattr(title, 'options'):
            ...
            title.style.css.font_size = self.page.body.style.globals.font.normal(
                options.get("title_notch", 20) + size_notch)
        div.add(title)
    ...
    def disclaimer(self, copyright=None, links=None, width=(100, '%'), height=("auto", ''), align="center",
                   options=None,
                   profile=None):
        """
    ...
    """
        ...
        row = self.page.ui.row([
            ...
            self.page.ui.input(
                placeholder="Last Name", html_code="%s_last_name" % html_code)]).css(
    ...
    """
        ...
        for logo in logos:
            img = self.page.ui.img(
                "%s?raw=true" % logo, path="https://github.com/epykure/ressources/blob/master/logos",
                width=("auto", ''))
