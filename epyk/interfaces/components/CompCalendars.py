...
class Calendar:
    ...
    """
        ...
        dfl_options = {
            ...
            'header': {'font-size': self.page.body.style.globals.font.normal(3),
        ...
        while start.month == month:
            ...
            days_data.append(
                {'today': today == start, "number": start.day, 'tasks': tasks_view, 'date': start.isoformat(),
    ...
    """
        ...
        for i, l in enumerate(labels):
            ...
            pie = self.page.ui.charts.chartJs.pie(
                record, y_columns=["capacity"], x_axis="name", html_code=html_code_chart, height=(150, "px"),
                options=options,
                profile=profile)
        ...
        component = self.page.ui.link("%s %s" % (icon.html(), text),
                                      self.page.js.objects.get(
                                          "%(url)s&text=%(task)s&dates=%(start)s/%(end)s&details=%(details)s&location=%(location)s" % {
                                              'url': google_url, "task": task, 'start': start, 'end': end,
    ...
    def agenda(self, task, start, end, details=None, location=None, icon: str = "calendar",
               text: str = "Add to Calendar",
    ...
    """
        ...
        events_options = {'DTSTART;VALUE=DATE': start, 'DTEND;VALUE=DATE': end, 'SUMMARY': task or '',
                          'LOCATION': location or '', 'DESCRIPTION': details or '', 'STATUS': 'CONFIRMED',
                          'SEQUENCE': 3}
        ...
        component = self.page.ui.links.data("<i style='font-size:%s;color:%s' class='%s'></i> %s" % (
            self.page.body.style.globals.font.normal(5), self.page.theme.greys[-1], icon, text),
                                            str_calendar % str_event,
